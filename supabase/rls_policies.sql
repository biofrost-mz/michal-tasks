...
-- Helper: returns workspace IDs where the current user is owner (stricter than admin).
...
-- Helper: workspace IDs where the current user may EDIT content (all roles
-- except viewer). Viewers keep SELECT access but cannot write.
...
-- Role escalation guard: nobody promotes to `owner` via a role change; a plain
-- admin may only set/target member|viewer, an owner may also manage admins but
-- not another owner's row. USING sees the OLD row, WITH CHECK the NEW row.
...
-- Self-leave allowed for non-owners; admins can remove member|viewer, owners can
-- also remove admins, but nobody deletes an owner row here (transfer ownership first).
...
-- Role gating mirrors create_workspace_invite: admins may only mint member|viewer
-- invites, owners may also mint admin invites, nobody mints owner invites.
