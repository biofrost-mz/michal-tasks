...
export default function RemoteErrorLogsPanel() {
  ...
  const handleClear = async () => {
    ...
    const confirmed = window.confirm("Opravdu vymazat tvoje vzdálené produkční logy? Lokální logy v prohlížeči tím zůstanou nedotčené.");
    if (!confirmed) return;
  ...
  return (
    <div
      style={{
        ...
        bottom: 72,
        zIndex: 9997,
        width: open ? "min(680px, calc(100vw - 32px))" : "auto",
    ...
    >
      {!open ? (
        <button
          ...
          title="Produkční chyby ze Supabase"
        >
          <span
            style={{
              width: 10,
              height: 10,
              borderRadius: "50%",
              background: logs.length ? SEVERITY_COLORS[worstSeverity] : "var(--green)",
              boxShadow: logs.length ? `0 0 12px ${SEVERITY_COLORS[worstSeverity]}` : "0 0 12px var(--green)",
            }}
          />
        ...
        >
          <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--border-soft)", display: "flex", justifyContent: "space-between", gap: 16 }}>
            ...
            <button
              ...
              aria-label="Zavřít produkční chyby"
            >
              ×
            </button>
          </div>

          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "12px 18px", borderBottom: "1px solid var(--border-soft)" }}>
              ...
              {["fatal", "error", "warning", "info"].map((severity) => (
                <span
                  ...
                  style={{
                    ...
                    border: "1px solid var(--border-soft)",
                    background: "var(--bg-2)",
                    color: severityCounts[severity] ? SEVERITY_COLORS[severity] : "var(--text-4)",
                    fontSize: 11.5,
                    fontWeight: 750,
                    fontFamily: "var(--mono)",
                  }}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button type="button" onClick={() => loadLogs()} disabled={loading} style={smallButtonStyle}>
                {loading ? "Načítám…" : "Obnovit"}
              </button>
              <button type="button" onClick={handleExport} style={smallButtonStyle}>Export</button>
              <button type="button" onClick={handleClear} style={{ ...smallButtonStyle, color: "var(--red)", background: "var(--red-soft)" }}>Vymazat</button>
            </div>
          </div>

          <div style={{ maxHeight: "440px", overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
            {!logs.length ? (
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                          ...
                          <span style={{ color: "var(--text)", fontSize: 13, fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {log.message}
                          </span>
                        </div>
                      ...
                      <span style={{ color: "var(--text-3)", fontSize: 11.5, fontFamily: "var(--mono)", flex: "0 0 auto" }}>
          ...
          <div style={{ padding: "9px 18px", borderTop: "1px solid var(--border-soft)", color: "var(--text-4)", fontSize: 11.5, fontFamily: "var(--mono)", display: "flex", justifyContent: "space-between" }}>
