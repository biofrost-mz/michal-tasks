...
function StatusPill({ meta, error }) {
  if (error) {
    return <span style={pillStyle("var(--red)", "var(--red-soft)")}>error</span>;
  }
  if (!meta) {
    return <span style={pillStyle("var(--text-3)", "var(--bg-2)")}>bez meta</span>;
  }
...
export default function AiTestConsolePanel() {
  ...
  return (
    <div
      style={{
        ...
        top: 140,
        zIndex: 10049,
        width: open ? "min(780px, calc(100vw - 32px))" : "auto",
        maxHeight: "min(780px, calc(100vh - 150px))",
    ...
    >
      {!open ? (
        ...
        >
          ...
          <div style={{ padding: 16, display: "grid", gap: 12, maxHeight: "calc(100vh - 250px)", overflowY: "auto" }}>
...
function ResultBlock({ title, value }) {
  return (
    <details open={title === "Parsed result" || title === "Meta"} style={{ border: "1px solid var(--border-soft)", borderRadius: 12, background: "var(--bg-2)", overflow: "hidden" }}>
      ...
      <pre style={{ margin: 0, padding: "10px 12px", borderTop: "1px solid var(--border-soft)", color: "var(--text-2)", fontFamily: "var(--mono)", fontSize: 11, lineHeight: 1.45, overflowX: "auto", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
        {stringify(value)}
      </pre>
    </details>
  );
